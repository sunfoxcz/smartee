...
namespace Sunfox\Smartee\Compiler;
...
class Compiler
{
    ...
    public function compile(array $tokens, $className)
    {
        ...
        return "<?php\n"
            ...
            . "use Sunfox\\Smartee\\Runtime as LSR;\n\n"
            . "class $className extends Sunfox\\Smartee\\Runtime\\Template\n{\n"
            . implode("\n\n", $members)
            . "\n\n}\n";
    }
    ...
    private function processHtmlAttributeBegin(Token $token)
    {
        ...
        if (in_array($lower, ['href', 'src', 'action', 'formaction'], true)
    ...
    public function expandMacro($name, $args, $modifiers = null, $nPrefix = null)
    {
        ...
        if ($nPrefix === MacroNode::PREFIX_INNER && !strcasecmp($this->htmlNode->name, 'script')) {
            $context = [Engine::CONTENT_HTML, self::CONTEXT_HTML_JS];
        } elseif ($nPrefix === MacroNode::PREFIX_INNER && !strcasecmp($this->htmlNode->name, 'style')) {
            $context = [Engine::CONTENT_HTML, self::CONTEXT_HTML_CSS];
        } elseif ($nPrefix) {
            $context = [Engine::CONTENT_HTML, self::CONTEXT_HTML_TEXT];
        } else {
            $context = [Engine::CONTENT_HTML, $this->context];
        }
